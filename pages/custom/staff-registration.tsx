    ...
    return(
        ...
        <div className="p-6 bg-white md:mx-auto">
          ...
          <div className="text-center">
            <h3 className="text-base font-semibold text-center text-gray-900 md:text-2xl">Registration Completed!</h3>
            <p className="my-2 text-gray-600">Thank you for completing your registration. You will be notified when website is ready.</p>
...
export function AvatarUpload({user, nextStep}: {user: {[key:string]: any}, nextStep: () => void}) {
  ...
  const uploadToServer = async () => {
    ...
    const res = await axios.post('/api/users/avatar-upload', {key: user.key, avatar: data.url});
    if(res.status === 200) {
      nextStep();
    }
  };
